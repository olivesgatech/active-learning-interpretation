...
class ActiveLearningMeter:
    def __init__(self, cycles=1, initial_training_sample=1, train_seismic, test_seismic, train_labels, test_labels, sampling_method='reconstruction'):
...
def run_active_learning(cycles=1, initial_training_sample=1, train_seismic, test_seismic, train_labels, test_labels, sampling_method='reconstruction'):
    ...
    for cycle in range(cycles):
        ...
        uncertainty_scores = []
        entropy_train = 0
        pred_segmentation_vol = np.zeros(seismic.shape)
        ground_truth_labels = np.zeros(seismic.shape)
        
        model.eval()
        with torch.no_grad():
            for i in range(train_seismic.shape[1]):
                
                section = torch.from_numpy(train_seismic[:,i,:]).to(device).type(torch.float).unsqueeze(0).unsqueeze(0)
                label = torch.from_numpy(train_labels[:,i,:]).to(device).type(torch.long).unsqueeze(0)
                ...
                pred_segmentation_vol[:,i,:] = out.argmax(1).detach().cpu().numpy().T.squeeze()
        ...
        with torch.no_grad():
            for i, (section, label) in enumerate(test_loader):
                out, _ = model(section)
                pred_test_vol[:, i,:] = out.argmax(1).detach().cpu().numpy().T.squeeze()
        ...
        print('Cycle No: {} | Mean Train IOU: {} | Mean Test IOU : {}'.format(cycle, IOU_train.mean(), IOU_test.mean()))
